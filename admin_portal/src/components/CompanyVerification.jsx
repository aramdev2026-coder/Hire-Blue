    ...
    <div className="space-y-4 max-w-4xl">
      ...
      <div className="flex gap-2">
        <button 
          ...
          className={`px-3 py-2 rounded-lg text-xs font-medium border transition-all ${filter === 'PENDING_VERIFICATION' ? 'bg-amber-600 text-white border-amber-700' : 'bg-white border-slate-200 text-slate-600'}`}
        >
          Pending
        </button>
        <button 
          onClick={() => setFilter('ACTIVE')}
          className={`px-3 py-2 rounded-lg text-xs font-medium border transition-all ${filter === 'ACTIVE' ? 'bg-emerald-600 text-white border-emerald-700' : 'bg-white border-slate-200 text-slate-600'}`}
        >
          Active
        </button>
        <button 
          onClick={() => setFilter('REJECTED')}
          className={`px-3 py-2 rounded-lg text-xs font-medium border transition-all ${filter === 'REJECTED' ? 'bg-rose-600 text-white border-rose-700' : 'bg-white border-slate-200 text-slate-600'}`}
        >
          Rejected
        </button>
      </div>
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-slate-50 text-xs font-semibold uppercase tracking-wider text-slate-500 border-b border-slate-200">
                ...
                {/* Only show Actions header if the active tab is Pending */}
            ...
            <tbody className="divide-y divide-slate-100 text-sm">
              {employers.map((employer) => (
                <tr key={employer.id}>
                  ...
                  {/* Only display functional control buttons if the active tab is Pending */}
