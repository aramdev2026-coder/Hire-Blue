            ...
            >
              ...
              {jobs.map((job) => (
                <option key={job.id} value={job.id}>
                  {job.roleTitle} - {job.employer.companyName}
                </option>
              ))}
            </select>
          </div>
              ...
              <div className="bg-white border border-slate-200 rounded-xl p-5 shadow-xs flex flex-col justify-between border-t-4 border-t-emerald-600">
                <div className="space-y-4">
                  ...
                  <div className="text-xs space-y-1 text-slate-600 bg-slate-50 border border-slate-100 p-3 rounded-lg">
                    ...
                    <p><strong>Target Boundary:</strong> {selectedJob.location}</p>
                    <p><strong>Experience Mandate:</strong> {selectedJob.expRequired}</p>
              ...
              <div className="space-y-3">
                ...
                {matchedCandidates.length === 0 ? (
                  ...
                  matchedCandidates.slice(0, 5).map((candidate) => (
                    <div key={candidate.id} className="border border-slate-200 rounded-lg p-3 flex justify-between items-center bg-emerald-50/40 border-emerald-100 hover:shadow-md transition-shadow">
                      <div>
                      ...
                      <button 
                        ...
                        className="bg-emerald-600 hover:bg-emerald-700 text-white px-3 py-1.5 rounded-md text-xs font-medium transition-colors cursor-pointer shadow-xs"
                      >
                        Shortlist
                      </button>
                    </div>
